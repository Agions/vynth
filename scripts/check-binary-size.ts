...
const budgetMb = Number(process.env.ZENO_SIZE_BUDGET_MB ?? DEFAULT_BUDGET_MB);
const binaryPath = resolve(process.cwd(), 'dist', 'zeno');
...
console.log(`[size-gate] dist/zeno = ${mb.toFixed(2)} MB (budget ${budgetMb} MB)`);
