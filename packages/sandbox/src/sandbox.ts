...
function safeResolve(target: string, cwd: string): string {
  ...
  if (abs !== cwd && !abs.startsWith(cwd + sep)) {
    throw new SandboxError(`path escapes sandbox: ${target}`);
  }
  ...
  if (realAbs !== realCwd && !realAbs.startsWith(realCwd + sep)) {
    throw new SandboxError(`path escapes sandbox via symlink: ${target}`);
  }
  return realAbs;
}
...
export async function readText(path: string, cwd: string): Promise<ToolResult> {
  try {
    ...
    return { ok: false, output: '', error: errMsg(err) };
  }
}
...
export async function runCommand(command: string, opts: RunOpts): Promise<ToolResult> {
  ...
  if (!opts.networkAllowed) {
    return { ok: false, output: '', error: 'network blocked by sandbox policy' };
  }
  return new Promise<ToolResult>((resolveResult) => {
    ...
    const timer = setTimeout(() => {
      proc.kill('SIGKILL');
      resolveResult({ ok: false, output: out, error: `timeout after ${timeout}ms` });
    }, timeout);
    ...
    proc.on('close', (code) => {
      ...
      resolveResult({ ok: code === 0, output, error: code === 0 ? undefined : `exit ${code}` });
    });
    proc.on('error', (e) => {
      clearTimeout(timer);
      resolveResult({ ok: false, output: '', error: errMsg(e) });
    });
  });
}
