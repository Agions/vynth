...
impl<'a> Widget for ChatBubble<'a> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        let role_style = Style::default().fg(accent).add_modifier(Modifier::BOLD);
        let dim_style = Style::default().fg(Color::DarkGray);
        let border_style = Style::default().fg(Color::DarkGray).bg(bg);
        let content_style = Style::default().fg(Color::White).bg(bg);
...
fn build_horizontal_border<'b>(
    ...
    if label.is_empty() {
        ...
        Line::from(Span::styled(plain, Style::default().fg(border_color)))
    } else {
        Line::from(vec![
            Span::styled(
                label_str,
