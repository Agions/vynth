...
All notable changes to Zeno are documented in this file. The format follows
...
- **TUI 全帧重绘 + 3 段式 IDE 布局**：废弃 DECSTBM 增量绘制 + SGR 颜色泄漏，
  ...
  4 行矩形输入框；移除角色标签（System/Zeno/You/Tool），改用左侧颜色条；
...
- **错误码 6 位体系（VC-XXXXXX）**：`packages/core/src/error-codes.ts`
  权威表（22 个已声明码 + 6 个族默认回退码）+ `isZenoErrorCode` / `fromLegacy`
  / `describe` 工具函数；`ZenoError` 基类新增 `numericCode: ZenoErrorCode`
...
- `packages/harness/src/e2e.test.ts`：CLI 退出码契约新增 6 位码正则断言；
  空 `ZENO_API_KEY` → exit 1 + `[VC-XXXXXX] missing ZENO_API_KEY` 断言。
- **Agent 引擎 + LLM（F4 / F6 / F7 / F8）**：`@zeno/engine` 提供 `runAgent`
...
- **内置工具 + 沙箱守卫（F5 / F10）**：`@zeno/sandbox` 的 `safeResolve`
  拒绝 `../` 越界、cwd 外绝对路径，以及对 realpath 后的 symlink 逃逸；
  `run_shell` 受 `ZENO_NET='0'` 阻断联网；`runCommand` / `readText` /
  `writeText` 全部经统一守卫。
- **插件无头接入（F9）**：`@zeno/plugins` 的 `loadPlugin` 动态 `import()`
  加载并校验 `pluginName` / `activate`；`loadAll` 批量激活并返回已加载名；
  6 例包级单测覆盖合法加载、批量加载、缺导出与无法 import 四类异常路径。
- **TUI 双模式契约（F2 / F3）**：`@zeno/tui` 的 `startTui` 与 CLI `runHeadless`
...
- 首次发布骨架：`@zeno/core` / `engine` / `sandbox` / `tui` / `plugins` /
  `mcp` / `harness` + `apps/cli`。无头 Agent 模式 (`zeno -g "<目标>"`)、
...
- **`EchoProvider` 类与 demo 模式已删除**：`createProvider(config)` 在
  `apiKey` 为空时**抛 `LlmError`**，不再静默 fallback。`ZENO_API_KEY` 为
  必填项。所有用户须先配置 API Key 才能运行 Zeno。
...
- 沙箱守卫覆盖 `..` 越界、绝对路径越界、symlink 逃逸三类逃逸路径
  （**F10 对抗 X3** 单测 100% 拦截），并经 `ZENO_NET='0'` 阻断 `run_shell`
  联网。
- gitleaks 密钥扫描入 CI 红线；体积门禁防回归。

### Test
- `bun test packages`: **69 pass / 0 fail**
  - `@zeno/core`: 21 例（loadConfig 默认值、ZENO_NET 解析、错误码层级、
    Emitter、log 分级、error-codes 8 例）
  - `apps/cli` e2e / `@zeno/harness`: 12 例（agent 流式工具、SSE 解析、
    plugin、EchoProvider、sandbox 守卫、symlink、联网拒止、明文 http 拒绝、
    CLI 退出码契约含 6 位码断言、CLI TUI 分流、空 API_KEY 抛 LlmError）
  - `@zeno/engine`: 8 例（无 Key 抛 LlmError、runAgent 流式与终止、maxSteps、
    OpenAI SSE token+tool_calls+usage、reasoning_content 解析、400 错误体读取、
    tool_call_id 消息构建、http 拒绝）
  - `@zeno/sandbox`: 7 例（cwd 内可读、../、绝对路径、symlink、writeText
    越界、ZENO_NET 开/关）
  - `@zeno/plugins`: 6 例（合法加载、批量加载、三类异常路径）
  - `@zeno/tui/theme`: 5 例（mocha/latte 色值、fg/bg/reset）
  - `@zeno/tui/stream-escape-hatch`: 5 例（首次不擦线、二次擦、clear 归零、
    空 clear、空串）
