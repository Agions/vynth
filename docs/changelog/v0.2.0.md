...
- **Demo 模式**：`VYNTH_API_KEY=""` 时自动切换 `EchoProvider`，离线体验
  流式输出 + 工具调用回填，开箱即跑。
- 默认 LLM：`https://api.deepseek.com/v1` + `deepseek-v4-pro`（与冻结裁决 X1/X2 一致）。
...
| `VYNTH_MODEL` | 模型名 | **`deepseek-v4-pro`**（v0.1.0 历史值为 `gpt-4o-mini`，v0.2.0 修正为冻结值） |
